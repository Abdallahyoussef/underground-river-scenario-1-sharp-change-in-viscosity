...
dy=[256*ones(1,3*150) 64*ones(1,150) 16*ones(1,150) 4*ones(1,8*150) 16*ones(1,150) 64*ones(1,150) 256*ones(1,3*150)]'; % length by ft of each grid in y direction
dy_cave=4;                                    % grid cell width inside cavity
...
K=[333*ones(((N-lx*6)/2),1);2.008E15*ones(lx,1); 5.023E15*ones(lx,1);6.526E15*ones(lx*2,1);5.023E15*ones(lx,1); 2.008E15*ones(lx,1);333*ones(((N-lx*6)/2),1)]; % horizontal permeability which is equal in x and y direction
phi=[0.18*ones(((N-lx*6)/2),1); ones(lx*6,1); 0.18*ones(((N-lx*6)/2),1)]; % porosity
Bc=1.127e-3; alpha=5.615;   % conversion factors in DE
...
for n = 2:((tmax/dt)+1)    % time loop
    ...
    for m=1:N
        ...
        if i<lx
            ...
            AP(F,F)=0.5*((1/Tx(m))+(1/Tx(m+1))); % Conservation of momentuem Dary's part  
        end
        if j>1
    ...
    Vol_T=Vol.*(phi_new.*bo_new.*Cp_v_old+(1-phi_new).*(rho_Solid/rho_Liquid)*Cp_Solid); % accumulation energy term
    ...
    for m=1:N % New temprature calculations
        j=ceil(m/lx);
        i=m-(j-1)*lx;
        AT(m,m)=Vol_T(m); % accumulation part
        if i>1
            F=m-1;
            F1=(i-1)+(j-1)*(lx-1);
...
figure(n)  % at 100 day
surf(vec2mat(Sol(L1+1:end,(100/dt)+1),lx));view(2);colormap jet; 
shading interp 
figure(n+1)  % at 200 day
surf(vec2mat(Sol(L1+1:end,(200/dt)+1),lx));view(2);colormap jet; 
shading interp 
